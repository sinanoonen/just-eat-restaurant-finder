...
function extractCuisines(raw: RawRestaurant): string[] {
    ...
    return raw.cuisines.map(cuisine => cuisine.name?.trim()).filter((name): name is string => !!name);
}
...
function formatAddress(raw: RawRestaurant): string {
    ...
    if (!address) return '';
    const parts = [address.firstLine?.trim(), address.city?.trim(), address.postalCode?.trim()].filter((part): part is string => !!part);
    return parts.join(', ');
}
...
export function parseRestaurants(raw: unknown): Restaurant[] {
    ...
    return response.restaurants.map(parseSingleRestaurant).filter((restaurant): restaurant is Restaurant => restaurant !== null);
}
